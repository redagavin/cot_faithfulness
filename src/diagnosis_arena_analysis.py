#!/usr/bin/env python3
"""
DiagnosisArena Gender Bias Analysis
Processes medical diagnosis cases with bidirectional gender swapping and LLM inference
"""
...
class DiagnosisArenaAnalyzer:
    def __init__(self):
        """Initialize the DiagnosisArena Analyzer"""
        self.dataset = None
        ...
        # CoT-enhanced prompt (DiagnosisArena format + CoT trigger)
...
Let's analyze step by step."""
        ...
        # Judge prompt for DiagnosisArena
...
Let's analyze step by step:"""
    ...
    def filter_and_prepare_cases(self):
        ...
        for i, example in enumerate(self.dataset):
            ...
            swapped_case_info = self.apply_gender_swap(example['Case Information'], gender)
            swapped_physical_exam = self.apply_gender_swap(example['Physical Examination'], gender)
            swapped_diagnostic_tests = self.apply_gender_swap(example['Diagnostic Tests'], gender)
            ...
            self.filtered_cases.append({
                ...
                'swapped_case_information': swapped_case_info,
                'swapped_physical_examination': swapped_physical_exam,
                'swapped_diagnostic_tests': swapped_diagnostic_tests,
    ...
    def extract_diagnosis_answer(self, response):
        ...
        # Priority 1b: \\boxed{} format (DiagnosisArena's expected format)
    ...
    def process_cases_with_models(self, sample_size=None):
        ...
        for model_name, model_path in models_config.items():
            ...
            # Format options string for prompt
            ...
            for i, case in enumerate(cases_to_process):
                ...
                swapped_prompt = self.cot_prompt.format(
                    case_info=case['swapped_case_information'],
                    physical_exam=case['swapped_physical_examination'],
                    diagnostic_tests=case['swapped_diagnostic_tests'],
                    options=format_options(case)
                )
            ...
            for i, result in enumerate(self.results):
                ...
                if gender == 'female':
                    ...
                    original_case_snippet = result['case_information']
                    modified_case_snippet = result['swapped_case_information']
                else:
    ...
    def save_to_spreadsheet(self, output_path="results/diagnosis_arena_results.xlsx"):
        """Save results to Excel spreadsheet with detailed statistics"""
        # Override output path for test script
        if len(self.results) <= 10:
            output_path = "tests/test_results/test_diagnosis_arena_results.xlsx"
    ...
    def run_complete_analysis(self, sample_size=None):
        ...
        print("Starting DiagnosisArena Analysis Pipeline...")
        print("=" * 60)
...
def main():
    ...
    print("\n- Results saved to diagnosis_arena_results.xlsx")
